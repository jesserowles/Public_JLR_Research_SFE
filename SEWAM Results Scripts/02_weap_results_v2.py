...
# v2 is for reading in April 2021 model run.
...
def read_weap_results(recalc=0, resave=0, ltonly = 0):
    ...
    ltfile = 'Reference Files/Performance by LOI Apr 2021.csv'
    wrfile = 'Reference Files/Daily Performance Apr 2021.csv'
    ...
    if (recalc == 1): #run through and recalculate everything
        ...
        lperr = pd.read_excel('2021.03.25 SFER LP Errors.xlsx',header=3) # read in sheet that has scenarios and errors from WEAP
        wr_raw = pd.read_csv('SFER_Complete_Results_File Apr 2021.csv') #read in complete daily results from WEAP
        wr = wr_raw.dropna() #remove columns with na's
        print('WEAP Performance table read, converting dates to Timestamps...')
        wr.index = pd.to_datetime(wr['Date'],format=' %m/%d/%Y') #format date as datetime, make index
        wr['Date'] = wr.index #reformat date column
        ...
        wr.loc[:,'Vol Eco'] = np.min(np.array([np.array(wr['Streamflow_CFS']/wr['Instream_Flow_Target_CFS']),
                                     ...
                                     axis=1) #ecological volumetric coverage
        wr.loc[:,'Vol Hum Local'] = np.min(np.array([np.array(wr['DT_Supplied_eWRIMS_Local_CFS']/wr['DT_Demand_e_Local_CFS']),
                                                     np.ones(len(wr.index))]).T,
                                           axis=1) #local human volumetric coverage

        wr.loc[:,'Vol Hum All'] = np.min(np.array([np.array(wr['DT_Supplied_eWRIMS_CFS']/wr['DT_Demand_e_CFS']),
                                                   np.ones(len(wr.index))]).T,
                                         axis=1) #all CA human volumetric coverage
        #Determine days where demands are met including 100%, 90%, 75%, and 50% of demands
        wr['Eco Met'] = wr['Streamflow_CFS'] >= wr['Instream_Flow_Target_CFS'] #1 or 0 for meeting ecological demands
        wr.loc[:,'Eco Met 90%'] = wr['Streamflow_CFS'] >= (wr['Instream_Flow_Target_CFS'] * 0.9) #90% ecological demand met
        wr.loc[:,'Eco Met 75%'] = wr['Streamflow_CFS'] >= (wr['Instream_Flow_Target_CFS'] * 0.75) #75% eco demands met
        wr.loc[:,'Eco Met 50%'] = wr['Streamflow_CFS'] >= (wr['Instream_Flow_Target_CFS'] * 0.5) #50% eco demands met

        wr.loc[:,'Hum Local Met'] = wr['DT_Supplied_eWRIMS_Local_CFS'] >= wr['DT_Demand_e_Local_CFS'] #local human demands met
        wr.loc[:,'Hum Local Met 90%'] = wr['DT_Supplied_eWRIMS_Local_CFS'] >= (wr['DT_Demand_e_Local_CFS']* 0.9) #90%
        wr.loc[:,'Hum Local Met 75%'] = wr['DT_Supplied_eWRIMS_Local_CFS'] >= (wr['DT_Demand_e_Local_CFS']* 0.75) #75%
        wr.loc[:,'Hum Local Met 50%'] = wr['DT_Supplied_eWRIMS_Local_CFS'] >= (wr['DT_Demand_e_Local_CFS']* 0.5) #50%

        wr.loc[:,'Hum All Met'] = wr['DT_Supplied_eWRIMS_CFS'] >= wr['DT_Demand_e_CFS'] #all CA human demands met
        wr.loc[:,'Hum All Met 90%'] = wr['DT_Supplied_eWRIMS_CFS'] >= (wr['DT_Demand_e_CFS'] * 0.9) #90%
        wr.loc[:,'Hum All Met 75%'] = wr['DT_Supplied_eWRIMS_CFS'] >= (wr['DT_Demand_e_CFS'] * 0.75) #75%
        wr.loc[:,'Hum All Met 50%'] = wr['DT_Supplied_eWRIMS_CFS'] >= (wr['DT_Demand_e_CFS'] * 0.5) #50%
